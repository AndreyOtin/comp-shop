...
function UseArrowNavigation(inputRef: InputRef, linkContainerRef: LinkContainerRef) {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (evt: globalThis.KeyboardEvent) => {
      ...
      const areUpCodes =
        (pressedKeys.has(Code.Tab) && pressedKeys.has(Code.ShiftLeft)) ||
        pressedKeys.has(Code.ArrowUp);
      const areDownCodes =
        (pressedKeys.has(Code.Tab) && !pressedKeys.has(Code.ShiftLeft)) ||
        pressedKeys.has(Code.ArrowDown);
      const isSearchInput = areUpCodes && activeLink.current === 0;

      const isNextElement =
        linkContainerRef.current.contains(document.activeElement.nextElementSibling) ||
        document.activeElement === inputRef.current;
      const isPrevElement =
        linkContainerRef.current.contains(document.activeElement.previousElementSibling) ||
        document.activeElement === inputRef.current;

      const isUp = areUpCodes && isPrevElement && activeLink.current > 0;
  ...
  return useCallback(() => {
    activeLink.current = -1;
    linkContainerRef.current?.scroll({ top: 0 });
  }, []);
}
